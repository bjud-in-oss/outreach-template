# Samordningsmotorn (SI v9.3)

Hej,

Denna samordningsmotor stärker AI Studios förmåga att bygga mer långsiktiga appar och flyttar utvecklarens fokus från byggandet av appen till appens användning.
...
- ⚙️ **[Systeminstruktion för Google AI Studio (SI v9.3)](AI_STUDIO_SYSTEM_INSTRUCTIONS.md)** – *Texten att klistra in i System Instructions.*
...
2. **Mekaniska kvalitetsspärrar (`scripts/verify-architecture.js`)**: Säkerställer max 120 rader per UI-vy, max 250 rader per fil, FSD-gränser och TDD-ordning.
3. **Fysiskt processminne (`doc/LAST_CYCLE/`)**: Tvingar AI-agenten genom ett transparent planeringssvep (1a -> 3c -> 4) innan kod skrivs.
4. **JIT Skill Library (`doc/skills/mattpocock/`)**: Komplett samling ingenjörs- och analysfärdigheter (`wayfinder`, `to-spec`, `tdd`, `implement`, m.fl.).
...
# 2. Initiera säkerhetshashar
npm run init-hashes

# 3. Kör arkitekturkontrollen
npm run verify
